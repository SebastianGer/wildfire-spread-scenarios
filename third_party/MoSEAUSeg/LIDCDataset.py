# Source: https://github.com/gaozhitong/MoSE-AUSeg/blob/main/data/lidc_dataset.py
...
class lidc_Dataset(Dataset):
    def __init__(self, root_dir, transform = None, label_range = 'all'):

        self.img_file = sorted((Path(root_dir) / 'images').iterdir())
        self.label_file = sorted((Path(root_dir) / 'labels').iterdir())
        self.transform = transform
        self.label_range = label_range
        
        self.imgs = [np.load(f)[None].astype(np.float32) for f in self.img_file]
        self.labels = [np.load(f) for f in self.label_file]
